...
INT32	CCSCfgMgr::Initalize(){
	...
	if (eNormal == n32LoadRet) n32LoadRet = LoadSkillModelLinkCfg();
	...
	if (eNormal == n32LoadRet) n32LoadRet = LoadDailyTaskConfig();
	if (eNormal == n32LoadRet) n32LoadRet = LoadInfiniteTaskConfig();
	Assert(eNormal==n32LoadRet);
	return n32LoadRet;
}

INT32	CCSCfgMgr::Uninitialize(){
	m_cNPCCfgMap.clear();
	m_cHeroCfgMap.clear();
...
INT32	CCSCfgMgr::LoadHeroHeadCfg()
{
...
INT32	CCSCfgMgr::LoadMapData(UINT32 un32MapID){
	m_cMapCfgSet.insert(un32MapID);
	return eNormal;
}

INT32 CCSCfgMgr::LoadMapLogicCfg(){
...
const CHAR* CCSCfgMgr::GetVoipServer(){
	return m_voipServerStr;
}

INT32 CCSCfgMgr::GetVoipPort(){
	return m_voipPort;
}

#define  SKILL_NEW_SWITCH_CFG 0

INT32 CCSCfgMgr::LoadNewSkillCfg(){
	...
	try{
		for (ptree::iterator iterInfo = cSkillCfg.begin(); iterInfo != cSkillCfg.end(); iterInfo++){
			...
			sTempCfg.eSkillType = (ESkillType)iterInfo->second.get<INT32>("SkillType", 0);
			...
			sTempCfg.bIfImpact = iterInfo->second.get<bool>("bIsInstant", 0);
...
INT32 CCSCfgMgr::LoadSkillModelAccountCfg(){
	...
	try{
		for (ptree::iterator iterInfo = cSkillCfg.begin(); iterInfo != cSkillCfg.end(); iterInfo++){
			...
			sTempCfg.sEffectInfo.n32EffectBaseValue = iterInfo->second.get<INT32>("eEffectValue", 0);
			...
			//读取伤害改变的基础值和千分比(1:500;2:300 的格式)
			string sBaseValueStr = iterInfo->second.get<string>("eEffectPlusRate");
			if(sBaseValueStr != "0"){
				typedef vector< string > split_vector_type;
				split_vector_type sBaseValueVec;
				boost::algorithm::split( sBaseValueVec, sBaseValueStr, boost::algorithm::is_any_of(";") );
				for (INT32 i = 0; i < sBaseValueVec.size(); ++i){
					string keyValuePairStr = sBaseValueVec[i];
					split_vector_type sKevValueVec;
					boost::algorithm::split( sKevValueVec, keyValuePairStr, boost::algorithm::is_any_of(":") );
					sTempCfg.sEffectInfo.eEffectAddCacuCate[i] = (EEffectCaculateCate)atoi(sKevValueVec[0].c_str());
					sTempCfg.sEffectInfo.eEffectAddCacuValue[i] = atoi(sKevValueVec[1].c_str());
				}
			}
			else{
				int abc = 0;
			...
			string sMultValueStr = iterInfo->second.get<string>("eEffectMultiplyRate");
			if(sMultValueStr != "0"){
				typedef vector< string > split_vector_type;
				split_vector_type sBaseValueVec;
				boost::algorithm::split( sBaseValueVec, sMultValueStr, boost::algorithm::is_any_of(";") );
				for (INT32 i = 0; i < sBaseValueVec.size(); ++i){
					string keyValuePairStr = sBaseValueVec[i];
					split_vector_type sKevValueVec;
					boost::algorithm::split( sKevValueVec, keyValuePairStr, boost::algorithm::is_any_of(":") );
					sTempCfg.sEffectInfo.eEffectMultCacuCate[i] = (EEffectCaculateCate)atoi(sKevValueVec[0].c_str());
					sTempCfg.sEffectInfo.eEffectMultCacuValue[i] = atoi(sKevValueVec[1].c_str());
				}
			}
...
INT32 CCSCfgMgr::LoadSkillModelLeadingCfg(){
...
INT32 CCSCfgMgr::LoadSkillModelEmitCfg(){
...
INT32	CCSCfgMgr::LoadSkillModelRangeCfg()
{
...
INT32	CCSCfgMgr::LoadSkillModelBufCfg()
{
	...
	try{
		for (ptree::iterator iterInfo = cSkillCfg.begin(); iterInfo != cSkillCfg.end(); iterInfo++){
			...
			sTempCfg.n32FlyEffectID = iterInfo->second.get<bool>("eFlyEffectID", 0);
...
INT32	CCSCfgMgr::LoadSkillModelSummonCfg()
{
...
INT32	CCSCfgMgr::LoadSkillModelMoveCfg()
{
...
INT32	CCSCfgMgr::LoadSkillModelSwitchCfg()
{
...
INT32	CCSCfgMgr::LoadSkillModelPurifactionCfg()
{
...
INT32	CCSCfgMgr::LoadSkillModelLinkCfg()
{
	const char *cpszfilename = "./CSBattleMgr/NewSkill/SkillCfg_links.xml";

	ptree cPtree;
	ptree cSkillCfg;
	try{
		read_xml(cpszfilename, cPtree, xml_parser::trim_whitespace);
		cSkillCfg = cPtree.get_child("SkillCfg_links");
	}catch(std::exception &cException){
		ELOG(LOG_ERROR, "open cfg file fail.");
		return eEC_OpenCfgFileFail;
	}
	...
	try{
		for (ptree::iterator iterInfo = cSkillCfg.begin(); iterInfo != cSkillCfg.end(); iterInfo++){
			...
			sTempCfg.eSkillModelType = eSkillEffectType_Link;
			sTempCfg.un32SkillModelID = iterInfo->second.get<UINT32>("<xmlattr>.un32ID", 0);
			if (0 == sTempCfg.un32SkillModelID){
				continue;
			}
			...
			sTempCfg.bIfCanBeBreak = iterInfo->second.get<bool>("bIsCanBeBreak", 0);
			sTempCfg.eStartPointType = (ELinkStartPointType)iterInfo->second.get<INT32>("eStartingPoint", 0);
			sTempCfg.bIfMoveBreak = iterInfo->second.get<bool>("bIsMoveBreak", 0);
			sTempCfg.n32FarthestDistance = iterInfo->second.get<INT32>("n32FarthestDistance", 0); 
			sTempCfg.n32LastTime = iterInfo->second.get<INT32>("n32LastTime", 0); 

			string sEvent = iterInfo->second.get<string>("TimeOverEventID", ""); 
			DoEventStr(sEvent, sTempCfg.asFinishModelList, c_n32MaxNextSkillEffectCfgNum); 

			sEvent = iterInfo->second.get<string>("BreakEventID", ""); 
			DoEventStr(sEvent, sTempCfg.asBreakModelList, c_n32MaxNextSkillEffectCfgNum); 

			m_cSkillModuleLinkCfgMap[sTempCfg.un32SkillModelID] = sTempCfg;
		}
	}catch(std::exception &cException){
		ELOG(LOG_ERROR, "read cfg file fail for %s (%d).", cException.what(), n32NewSkillNum);
		return eEC_ReadCfgFileFail;
	...
	ELOG(LOG_DBBUG, " Load %d Skill cfg.", (UINT32)m_cSkillModuleLinkCfgMap.size());
	return eNormal;
}


INT32	CCSCfgMgr::LoadPassitiveSkillCfg()
{
...
INT32	CCSCfgMgr::LoadPassitiveEffectBloodSeekCfg()
{
...
INT32	CCSCfgMgr::LoadPassitiveEffectReboundCfg()
{
...
INT32	CCSCfgMgr::LoadPassitiveEffectReliveCfg()
{
...
void	CCSCfgMgr::DoEventStr(std::string & str, SSNextSkillEffectCfg *asSkillModelList,INT32 num)
{
	if (str.length() < 1)
	{
		return;
...
//注:不可能有多层依赖，约定最多1层依赖(最多一层())
#define SPILIT_STR_FEATURE_SEM ";"
#define SPILIT_STR_FEATURE_COM "," 

void  CCSCfgMgr::SpilitEventStr(std::string & str, std::vector<SSNextSkillEffectCfg>& tSNSE )
{
...
INT32 CCSCfgMgr::LoadFilterWordsCfg(){
...
INT32 CCSCfgMgr::LoadHeroChooseCfg(){
...
VOID CCSCfgMgr::DoLoadRobotAICfg(ERobotAIType eAiType, const char* fileName){
	ptree cPtree;
	ptree cHeroCfg;
	try{
		read_xml(fileName, cPtree, xml_parser::trim_whitespace);
	...
	try{
		for (ptree::iterator iterInfo = cHeroCfg.begin(); iterInfo != cHeroCfg.end(); iterInfo++){
			...
			sTempCfg.n32AiType = eAiType;
			sTempCfg.n32ModelID = iterInfo->second.get<INT32>("n32FunctionID");
			sTempCfg.n32NodeType = iterInfo->second.get<INT32>("n32NodeID");
...
INT32 CCSCfgMgr::LoadRobotAICfg(){
	m_cRobotAICfg.clear();
	
	DoLoadRobotAICfg(eRobotAIType_Guide,"./CSBattleMgr/RobotAICfg.xml");
	DoLoadRobotAICfg(eRobotAIType_PvE,"./CSBattleMgr/RobotAICfg.xml");
	DoLoadRobotAICfg(eRobotAIType_Match,"./CSBattleMgr/RobotAICfg.xml");

	ELOG(LOG_DBBUG, "Load %d RobotAI cfg.", (UINT32)m_cRobotAICfg.size());
	return eNormal;
}

INT32 CCSCfgMgr::LoadRobotAIPathCfg(){
...
INT32 CCSCfgMgr::LoadAiRobotName()
{
...
INT32 CCSCfgMgr::LoadDailyTaskConfig()
{
	const char *cpszfilename = "./CSBattleMgr/DailyTask.xml";
	//////////////////////////////////////////////////////////////////////////
	{
		STaskConfig &cTask = m_cTaskConfigs[10001];
		cTask.taskID = 10001;
		cTask.taskTimeType = TTT_Daily;
		cTask.taskType = TT_GamePlay;
		cTask.taskMaxCount = 3;
	}
	//////////////////////////////////////////////////////////////////////////
	{
		STaskConfig &cTask = m_cTaskConfigs[10002];
		cTask.taskID = 10002;
		cTask.taskTimeType = TTT_Daily;
		cTask.taskType = TT_GameWin;
		cTask.taskMaxCount = 1;
	}
	//////////////////////////////////////////////////////////////////////////
	return eNormal;
}

INT32 CCSCfgMgr::LoadInfiniteTaskConfig()
{
	const char *cpszfilename = "./CSBattleMgr/InfiniteTask.xml";
	//////////////////////////////////////////////////////////////////////////
	{
		STaskConfig &cTask = m_cTaskConfigs[20001];
		cTask.taskID = 20001;
		cTask.taskTimeType = TTT_Infinite;
		cTask.taskType = TT_KillMonster;
		cTask.taskMaxCount = 100;
	}
	//////////////////////////////////////////////////////////////////////////
	return eNormal;
}

STaskConfig* CCSCfgMgr::GetTaskConfig( UINT32 taskId )
{
	auto it=m_cTaskConfigs.find(taskId);
	if (it!=m_cTaskConfigs.end())
		return &(it->second);
	return NULL;
}
