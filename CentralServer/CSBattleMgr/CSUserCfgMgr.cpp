...
{
	const char *cpszfilename = "./CSBattleMgr/IAPShop/RuneCfg.xml";
	ptree cPtree;
	ptree cRunesCfg;
	try{
...
{
	const char *cpszfilename = "./CSBattleMgr/UserAccountCfg.xml";
	ptree cPtree;
	ptree cUserExpCfg;
	try{
		read_xml(cpszfilename, cPtree, xml_parser::trim_whitespace);
...
{
	const char *cpszfilename = "./CSBattleMgr/IAPShop/dailybonus.xml";
	ptree cPtree;
...
{
	const char *cpszfilename = "./CSBattleMgr/IAPShop/OtherItemCfg.xml";
	ptree cPtree;
	ptree OtherItemCfg;
	try{
		read_xml(cpszfilename, cPtree, xml_parser::trim_whitespace);
...
{
	const char *cpszfilename = "./CSBattleMgr/RuneSlotCfg.xml";
	ptree cPtree;
	ptree cRunesSlotCfg;
	try{
		read_xml(cpszfilename, cPtree, xml_parser::trim_whitespace);
...
INT32 CCSCfgMgr::LoadHeroBuyCfg(){
	const char *cpszfilename = "./CSBattleMgr/IAPShop/HeroBuyCfg.xml";
	ptree cPtree;
	ptree cHeroBuyCfg;
	try{
		read_xml(cpszfilename, cPtree, xml_parser::trim_whitespace);
...
INT32 CCSCfgMgr::LoadDiscountCfg(){
	const char *cpszfilename = "./CSBattleMgr/IAPShop/DiscountsCfg.xml";
	ptree cPtree;
	ptree cDiscountCfg;
	try{
		read_xml(cpszfilename, cPtree, xml_parser::trim_whitespace);
...
INT32	CCSCfgMgr::LoadInvalidWordCfg()
{
	...
	const char *cpszfilename = "./CSConfig/NamedFilterWorld.ini";

	char szWord[128]={0};

	ifstream    FileStream; 

	FileStream.open( cpszfilename ); 
...
{
	m_GuideEndAward.clear(); 
	const char *cfg = "./CSBattleMgr/CS_Award.xml"; 
...
INT32	CCSCfgMgr::LoadUserDBSaveCfg()
{
	...
	const char *cpszfilename = "./CSConfig/CSGlobalCfg.cfg";
	ifstream    FileStream; 
...
INT32 CCSCfgMgr::LoadHotAndNewGoodsCfg(){
	const char *cpszfilename = "./CSBattleMgr/IAPShop/HotAndNewCfg.xml";
	ptree cPtree;
	ptree cHeroBuyCfg;
	try{
		read_xml(cpszfilename, cPtree, xml_parser::trim_whitespace);
...
{
	const char * cfg = "./CSConfig/DBConfig.xml";
	TiXmlDocument doc(cfg); 
	if(doc.LoadFile())
	{
		TiXmlHandle dochandle(&doc);
